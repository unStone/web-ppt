...
export function structuralPatchStage(doc: EditDoc, patches: readonly Patch[]): EditDoc {
  ...
  const clonedMasters = new Set<string>();
  ...
  const cloneMaster = (id: string): void => {
    if (clonedMasters.has(id) || !stage.masters[id]) return;
    stage.masters[id] = structuredClone(stage.masters[id]);
    clonedMasters.add(id);
  };
  const cloneParent = (id: string): void => {
    if (stage.slides[id]) cloneSlide(id);
    else if (stage.layouts[id]) cloneLayout(id);
    else if (stage.masters[id]) cloneMaster(id);
  ...
  const cloneOwningCanvas = (parent: string): void => {
    ...
    else if (doc.masters[parent]) cloneMaster(parent);
    else if (doc.elements[parent]) {
      const canvas = canvasTargetOfElement(doc, parent);
      if (canvas.kind === 'slide') cloneSlide(canvas.id);
      else if (canvas.kind === 'layout') cloneLayout(canvas.id);
      else cloneMaster(canvas.id);
    }
  };
