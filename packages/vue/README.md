...
Layout and master designers use `createDesignEditor(container, snapshot.value.session, { target })` from the opt-in
`@web-ppt/editor/design` entry; `DesignEditor`, `DesignTarget`, `LayoutCatalogItem`, `LayoutDesignState`,
`MasterCatalogItem`, and `MasterDesignState` are re-exported for typed Vue-owned UI controls.
