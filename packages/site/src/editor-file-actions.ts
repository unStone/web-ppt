...
export function createEditorFileActions(options: EditorFileActionsOptions): EditorFileActions {
  ...
  const run = async (failure: '保存失败：{detail}' | '导出失败：{detail}', action: () => Promise<void>): Promise<void> => {
    ...
    try {
      ...
      options.notice(message(failure, { detail: error instanceof Error ? error.message : String(error) }), 'error');
    } finally {
      busy = false;
      options.onBusyChange();
    }
  ...
  return {
    ...
    exportImages(session, name, showComments = false) {
      return run('导出失败：{detail}', async () => {
        ...
        const presentation = session.toPresentation();
        options.notice(message('正在导出当前文稿的图片 ZIP…'));
        ...
        options.notice(message('已导出当前编辑态的图片 ZIP'), 'success');
      });
    },
  };
}
