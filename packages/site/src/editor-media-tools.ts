...
async function readBytes(file: File | undefined, maximum: number, label: string): Promise<Uint8Array> {
  if (!file) throw new Error(`请选择${label}`);
  if (!file.size || file.size > maximum) throw new Error(`${label}必须非空且不超过 ${maximum / 1024 / 1024} MiB`);
...
async function readPoster(file: File | undefined): Promise<MediaPoster | undefined> {
  ...
  if (!['image/png', 'image/jpeg', 'image/gif', 'image/webp'].includes(file.type)) {
    throw new Error('海报仅支持 PNG、JPEG、GIF 或 WebP');
  }
...
export function showMediaTools(
  context: () => MediaContext, notice: Notice, onClose: () => void,
): (() => void) | undefined {
...
</style><h2 id="mediaDialogTitle">音视频与海报</h2>
<form id="mediaForm">
<label>操作<select id="mediaAction"><option value="insert">插入新媒体</option><option value="poster">替换选中媒体的海报</option></select></label>
<small id="mediaTargetHint"></small>
<div id="mediaInsertionFields">
<label>媒体类型<select id="mediaKind"><option value="audio">音频 · PCM WAV</option><option value="video">视频 · MP4</option></select></label>
<label>媒体来源<select id="mediaSource"><option value="embedded">本机文件（嵌入）</option><option value="external">HTTP(S) 外链</option></select></label>
<label id="mediaFileField">媒体文件<input id="mediaFile" type="file" accept=".wav,audio/wav" /></label>
<label id="mediaUrlField" hidden>外链地址<input id="mediaUrl" type="url" placeholder="https://example.com/media" /></label>
<small id="mediaSourceHint">文件最大 25 MiB，保存在 PPTX 内，不会上传或转码。</small>
</div>
<label>海报图片<input id="mediaPoster" type="file" accept="image/png,image/jpeg,image/gif,image/webp" /></label>
<small id="mediaPosterHint">音频可省略海报，使用内置喇叭图标；图片最大 5 MiB。</small>
<p id="mediaError" role="alert"></p>
<div class="media-actions"><button id="closeMediaDialog" class="button" type="button">取消</button><button id="insertMedia" class="button primary" type="submit">插入</button></div>
</form>`;
  ...
  get('mediaTargetHint').textContent = canReplace ? `当前选中：${record.src.name || '媒体'}`
    : '选中一个未锁定的媒体对象后可替换海报；兼容外壳内容保持原样。';
  ...
  const sync = (): void => {
    ...
    submit.textContent = replacing ? '替换海报' : '插入';
    ...
    get('mediaSourceHint').textContent = external
      ? '外链不会下载或嵌入 PPTX；播放时会访问该地址，需要网络，且不保证浏览器支持其编码。'
      : '文件最大 25 MiB，保存在 PPTX 内，不会上传或转码。';
    get('mediaPosterHint').textContent = replacing ? '仅替换海报，不改变音视频源；图片最大 5 MiB。'
  ...
  const close = (): void => {
    ...
    preview?.destroy(); dialog.close(); dialog.remove();
    onClose();
  };
  ...
  get<HTMLFormElement>('mediaForm').onsubmit = (event) => {
    ...
    void (async () => {
      ...
      if ((video || replacing) && !poster) throw new Error(replacing ? '请选择新的海报' : '视频必须提供海报');
      ...
      if (!current.writable || current.session !== session || current.view !== view || view.slideId !== slideId) {
        throw new Error('文稿、页面或编辑模式已变化，请关闭后重新选择媒体');
      }
      if (replacing) {
      ...
      notice(video ? '已插入视频' : '已插入音频', 'success');
    })().catch((failure: unknown) => {
      if (dialog.open) error.textContent = failure instanceof Error ? failure.message : String(failure);
