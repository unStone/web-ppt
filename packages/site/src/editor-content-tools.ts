...
export function bindContentTools(context: () => ContentContext, notice: SiteNotice): void {
  const run = async (action: (owner: ContentContext) => void | Promise<void>): Promise<void> => {
    const owner = context(), slideId = owner.view?.slideId;
    try { await action(owner); } catch (error) {
      if (owner.view !== context().view || slideId !== owner.view?.slideId) return;
  ...
  buttons.addImage.addEventListener('click', () => void run(async ({ view }) => {
    if (!view) return;
    const id = await view.chooseImage();
    if (id && context().view === view) notice(message('图片已插入'), 'success');
  }));
