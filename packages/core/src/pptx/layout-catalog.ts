...
export interface MasterCatalogPath {
...
export function layoutCatalogPaths(
  ...
  return masterCatalogPaths(
    presentation, presentationRelationships, xml, relationships,
  ).flatMap((master) => master.layoutIds);
}

/** 母版身份与直属版式必须从关系图读取，不能依赖编号或文件名约定。 */
export function masterCatalogPaths(
  presentation: Element,
  presentationRelationships: LayoutRelationshipIndex,
  xml: (path: string) => Element | null,
  relationships: (path: string) => LayoutRelationshipIndex,
): MasterCatalogPath[] {
  const masters: MasterCatalogPath[] = [];
  ...
  for (const masterId of kids(kid(presentation, 'sldMasterIdLst'), 'sldMasterId')) {
    ...
    if (!masterPath || seen.has(masterPath)) continue;
    seen.add(masterPath);
    ...
    for (const layoutId of kids(kid(masterRoot, 'sldLayoutIdLst'), 'sldLayoutId')) {
      ...
      if (part && !layoutIds.includes(part)) layoutIds.push(part);
    }
    masters.push({ part: masterPath, layoutIds });
  }
  return masters;
}
