...
/**
 * DrawingML 几何 → SVG path（局部坐标 0,0-w,h）。
 * - presetGeom：手写预设形状表（~140 个）
 * - custGeom：完整支持 gdLst 公式求值（DrawingML guide 表达式语言）
...
function ep(cx: number, cy: number, rx: number, ry: number, deg: number): Pt {
