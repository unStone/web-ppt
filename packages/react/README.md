...
For a new-deck picker, lazy-load `@web-ppt/react/templates`. It forwards the shared frozen catalog and
`createPptxFromTemplate()` without adding recipes to the React component entry.

```ts
const templates = await import('@web-ppt/react/templates');
...
```ts
const charts = await import('@web-ppt/react/chart');
...
Layout and master designers use `createDesignEditor(container, snapshot.session, { target })` from the opt-in
`@web-ppt/editor/design` entry; `DesignEditor`, `DesignTarget`, `LayoutCatalogItem`, `LayoutDesignState`,
`MasterCatalogItem`, and `MasterDesignState` are re-exported for typed React-owned UI controls.
