...
- 新增 `@web-ppt/fonts/glyphs` 可选字体 Provider 及 HarfBuzz、Worker、FontFace 适配：验证原始容器与嵌入限制，返回定位字形、轮廓和 UTF-16 原文簇；整形器和字体字节由宿主提供。
  产品编辑器由 Cordis 文稿服务检查、应用和释放字体，提供中英文缺字诊断与本机替换；原始字体模式避免主线程重复解压，替换和安装失败会归还旧字体预算。当前字体资源用于预览及图片 / SVG 导出，不写入 PPTX。
...
- 表格 tombstone 与完整合并真值让删除只改变可见投影；被合并覆盖的文字和格式休眠而不销毁，拆分、撤销及
  ...
  发布入口为 12.20KB gzip；含静态分块。契约测试使用的完整薄包在排除 peer 后为 14,333B gzip，两种口径不混用。
