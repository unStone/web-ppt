...
const sentinel = '音频不是完整的 PCM WAV 文件';
const seen = new Set();
function inspect(path) {
  if (seen.has(path)) return;
  seen.add(path);
  const source = readFileSync(path, 'utf8');
  if (source.includes(sentinel)) throw new Error(`媒体上传校验静态泄漏：${path}`);
...
if (!entry.includes(sentinel) || !entry.includes('createMediaEditor')) throw new Error('media 子入口缺少上传实现');
