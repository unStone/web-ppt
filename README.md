...
| [`@web-ppt/editor`](packages/editor) | 编辑会话、原生 SVG 点选/框选、键盘遍历/微移、移动/缩放/旋转、智能吸附与三层增量 DOM 视图，无 UI 框架依赖 | `core` + `edit-core` + `viewer-core` | 17.9KB |
...
| `npm run test:edit` | 编辑模型 / 保留型 XML / OPC / 变换保存 245 项断言 + M1 11 项独立验收 + 32 份固件、244 对独立进程 SVG 指纹 |
| `npm run test:editor` | 105 项会话 / 增量 DOM / 点选与框选 / 键盘遍历与微移 / 坐标、移动、缩放、旋转与吸附 / 资源断言 + 真实 Chrome 命中、矩阵、可信键盘、pointer capture 与性能门禁 |
