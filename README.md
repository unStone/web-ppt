...
| [`@web-ppt/edit-core`](packages/edit-core) | 稳定身份、命令历史、编辑覆盖、增量保存与高保真投影，无框架无 DOM | `@web-ppt/core` | 76.79KB |
...
编辑模型主入口连静态共享 chunk 为 76.79KB gzip，首次调用保存再按需增加 8.30KB。
...
| `npm run test:edit` | 编辑模型 1065 项 + 保存 500 项 + PowerPoint 证据 9 项 + 79 份固件、514 对独立进程 SVG 指纹 |
