...
| [`@web-ppt/editor`](https://github.com/unStone/web-ppt/tree/master/packages/editor) | Editing session, native SVG selection, coordinates/handles/dragging, and incremental three-layer DOM. No UI framework. | `core` + `edit-core` + `viewer-core` | 7.6 KB |
...
Drag an element—or any member of the current multi-selection—to move it. Frames touch only transient ghost DOM;
pointer-up creates one undoable, saveable `SetXfrm` transaction.
...
| `npm run test:editor` | 34 session/incremental DOM/native selection/coordinate/handle/drag/resource assertions + real-Chrome hit-testing, matrix, pointer-capture, and performance gates |
