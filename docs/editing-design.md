...
| 切换效果 | `SetTransition{ slideId, t }` | `p:transition` / `p14:` / `p159:morph` | 已实现 40 种效果、view/edit 预览与保留型写回 | P2 |
| 元素动画 | `SetAnimations{ slideId, steps }` | `p:timing` 时间树 | 已实现 128 步安全上限、效果目录、运动路径、来源只读边界与保留型写回 | P2 |
...
| **M5** | 打磨 | 格式刷、查找替换、选择窗格、锁定、自动保存与崩溃恢复、切换效果编辑（已完成） | 崩溃恢复用例通过；性能预算全部达标 |
| **M6** | 扩展 | 动画编辑已完成；顶点编辑、表样式、协同适配包继续独立推进 | 各自独立验收 |
