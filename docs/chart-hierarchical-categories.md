...
| 跨文稿复制 | 经典图表仍要求目标有相同 OPC 资源闭包；缺少闭包时原子拒绝，不将其转换成普通图形 |
...
| 指标 | 结果 |
|---|---|
| 图表按需入口 | 77,898B 原始 / 24,737B gzip，排除既有 peer 与 fflate；原 25,000B gzip 预算不变 |
| 冷导入 / 首次查询 | 14.1ms / 10.7ms，冷导入含尚未加载的依赖 |
| 编辑并保存 | 中位 16.1ms，p95 35.3ms |
| 采样内存增量 | heap 114,758,584B / RSS 256,000,000B；未强制 GC，包含尚未回收的临时分配，不表示保留量或浏览器峰值 |
