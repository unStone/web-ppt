...
| 国际化 | ⚠️ 官网三页静态中英文、SEO、原地切换，编辑文件/模板/本机恢复、首页动态展示、查看器/样本预览、媒体、图表、查找替换/格式刷、页面备注/切换/动画时间线、对象格式/图片裁剪/链接、常用插入、187 种形状名称与动画预览/长按状态已接入；SDK 生成的无障碍名称与完整语言矩阵仍待完成，发布包不含站点词条 |
