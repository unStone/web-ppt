...
- [005 矢量 PDF](tickets/005-vector-pdf.md)进行中：可选入口、文字和基础图形、页任务、首批图片/裁剪、透明度/渐变及对象级滤镜回退已有 source / dist 与独立读取证据；已补基础文字装饰、字体失败定位、首批 SVG 覆盖检查、原生表格 / 圆形 / 箭头、几何图案及图片填充 / 平铺 / 背景。首批图片规范化已通过浏览器与独立像素对照；图案的阅读器栅格差异保留。Cordis 产品入口、字体选择和导出资源消费已接入；其余图形、文字效果与完整产品验收继续推进，见[实现进度](vector-pdf-implementation.md)。
