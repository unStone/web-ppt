...
| 失败和局部回退 | 首批滤镜、小型大写和弧形艺术字可局部回退并返回定位，缺字体 / 缺字和嵌入拒绝可定位；已接入节点 / 属性 / CSS 检查，允许项值域及引用定义的完整审计、其余效果和产品字体来源说明仍待完成。开发入口尚未进入产品支持矩阵 |
| 字体与资源验证 | 继续扩大文字位置、CJK 挤压、组合簇映射、其他字体诊断、资源限制、释放及长文稿成本验证 |
| 结构整理 | 矢量与图片写入器目前有对象写入、批注、页任务重复；代码审查阶段收敛共享职责，区分共享的字节/图像处理与各自的页面内容 |
| 产品与门禁 | 本轮 check → test → build → verify 已通过；005 的 Cordis 文稿服务接入、完整中英文产品入口、导出生命周期、矢量 source/dist 常规门禁整合、运行成本及完成后最终复验仍待完成 |
