...
- 当前自动证明 49/49 份可编辑 PPTX 无编辑保存逐字节同一；单形状移动只改变
...
- `npm run test:edit:m1` 的保存契约 253 项全绿；单一清单的 27/27 份当前 Office 产物均由
  LibreOffice 无修复/恢复诊断地打开、核对页数并导出 PDF，覆盖移动、删除、层级、剪贴板、文字、表格、
  新增/移动/删除/复制页面、格式、二维效果、图片替换裁剪与元素/文字超链接。
...
- 修复后精确执行 `npm run check && npm test && npm run build`：core 2125、edit-core 661、保存 253、
  PowerPoint 报告 9、editor 283、57 份固件 / 172 页 / 344 对独立 SVG 指纹、metafile 130 全绿，五个发布包构建成功；
...
- 当前 macOS 环境既无 Windows runner，也没有安装 Mac 桌面 PowerPoint，因而不能产生真实成功报告。
  ...
  27/27 成功结果，不能把“门禁设施已就绪”偷换成“PowerPoint 已验收”。
