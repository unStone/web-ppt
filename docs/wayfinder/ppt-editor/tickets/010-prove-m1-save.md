...
- 已自动证明 37/37 份可编辑 PPTX 无编辑保存逐字节同一；单形状移动只改变
...
- 保存测试以单一清单证明 15/15 份产物齐全并记录预期页数；`npm run test:edit:m1` 已让 LibreOffice
  无修复/恢复诊断地逐份打开、校验页数并导出 PDF，M1 共 90 项断言。
- PowerPoint COM 验收器读取同一清单，设置 `DisplayAlerts = ppAlertsAll`，并以
  `Open2007(..., OpenAndRepair = msoFalse)` 逐份打开、校验页数。当前环境没有 Windows 桌面 PowerPoint，仍需在真机运行
