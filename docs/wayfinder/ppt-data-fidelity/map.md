...
- [让官网完整支持中英文](tickets/006-site-i18n.md)正在接入：三页静态镜像、语言优先级、原地切换与双语 SEO
  已实现；标题/文稿类型/撤销重做、编辑文件任务与转换确认、按需模板及本机恢复接入消息绑定，
  首页查看器/样本预览状态、架构展示、预览与全屏语言入口，以及媒体工具/试听/校验、图表数据工具、
  查找替换/格式刷、页面备注/切换/动画时间线、对象格式/图片裁剪/链接、常用插入与页面导航名称也已接通；
  187 种形状可读名称、动画预览/长按状态、通用错误摘要、选择窗格/搜索/文字编辑/图片选择无障碍名称
  及空占位符提示也已接入；完整语言矩阵仍待验收。
