...
|---|---:|---:|---|
| 页面启动 | 19,014 | 6,154 | 2,254,902 / 509,849 |
| 首次激活额外加载的应用（含 Cordis） | 2,300,702 | 538,169 | 纳入下行总量 |
| 页面启动与首次激活合计 | 2,319,716 | 544,323 | 2,397,467 / 544,349 |

测量来自官网浏览器门禁的 `out/site-editor-browser/cordis-size.json`。页面状态及上游依赖移入应用，首屏减少不等于首次打开总成本消失。合计预算由原首屏预算加应用侧实测增量（当前 gzip +34,500 B）得出；不能只检查变小后的入口。
